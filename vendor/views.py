...
class all_bookings_count(viewsets.ViewSet):
    ...
    def list(self, request):
        ...
        if user.is_doctor:
            ...
            combined = {
                "accepted": to_status_dict(consultation_counts)['accepted'] +
                            to_status_dict(online_counts)['accepted'] +
                            to_status_dict(test_counts)['accepted'] +
                            to_status_dict(vaccination_counts)['accepted'],

                "completed": to_status_dict(consultation_counts)['completed'] +
                             to_status_dict(online_counts)['completed'] +
                             to_status_dict(test_counts)['completed'] +
                             to_status_dict(vaccination_counts)['completed'],

                "cancelled": to_status_dict(consultation_counts)['cancelled'] +
                             to_status_dict(online_counts)['cancelled'] +
                             to_status_dict(test_counts)['cancelled'] +
                             to_status_dict(vaccination_counts)['cancelled'],
            }

            return Response({"total": combined})

        elif user.is_daycare:
            try:
                daycare_instance = day_care.objects.get(user=user)
            except day_care.DoesNotExist:
                return Response({"error": "Daycare profile not found."}, status=status.HTTP_400_BAD_REQUEST)

            daycare_counts = day_care_booking.objects.filter(
                daycare=daycare_instance
            ).values('status').annotate(total=Count('id'))

            status_counts = to_status_dict(daycare_counts)

            return Response({"total": status_counts})

        elif user.is_service_provider:
            # Assuming user has bookings as a service provider
...
from django.utils import timezone
from datetime import time   # ✅ time object (not module)
